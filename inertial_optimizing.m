...
for dir = 0:1:7
    for start = 0:5:135
        for fin = start:5:135
            %% defines burn
            burnlength = year; % s
            for step = (start/135)*burnlength*TR+1:(fin/135)*burnlength*TR+1
                if dir == 0
                    thrust_profile(step, 1) = 1; % anti-velocity (forward)
                ...
                elseif dir == 1
                    ...
                    thrust_profile(step, 2) = 1; % velocity (backward)
                    thrust_profile(step, 3) = 0; % left velocity (rightward)
                    thrust_profile(step, 4) = 0; % right velocity (leftward)                        
                elseif dir == 2
                    thrust_profile(step, 1) = 0; % anti-velocity (forward)
                    thrust_profile(step, 2) = 0; % velocity (backward)
                    thrust_profile(step, 3) = 1; % left velocity (rightward)
                    thrust_profile(step, 4) = 0; % right velocity (leftward)
                elseif dir == 3
                    thrust_profile(step, 1) = 0; % anti-velocity (forward)
                    thrust_profile(step, 2) = 0; % velocity (backward)
                    thrust_profile(step, 3) = 0; % left velocity (rightward)
                    thrust_profile(step, 4) = 1; % right velocity (leftward)
                elseif dir == 4
            ...
            syms z;
            Index = 1 + (fin-start)/5 + symsum(28-z,z,0,(start/5)-1) + dir*406;
            Minimum_store(Index, 1) = dir;
            Minimum_store(Index, 2) = start;
            Minimum_store(Index, 3) = fin;
            Minimum_store(Index, 4) = MinL4_rel_vel;
            Minimum_store(Index, 5) = I_MinL4_rel_vel;
            Minimum_store(Index, 6) = L4_p_difference_magnitude_store(I_MinL4_rel_vel);
            Minimum_store(Index, 7) = MinL5_rel_vel;
            Minimum_store(Index, 8) = I_MinL5_rel_vel;
            Minimum_store(Index, 9) = L5_p_difference_magnitude_store(I_MinL5_rel_vel);
        end
    end
end
